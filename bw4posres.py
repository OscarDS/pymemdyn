...
# distribution. Usually the path where biopython is installed will look
# something like the following path: /lib/python2.7/site-packages/Bio
# For now we are just extending the dictionary to the various possible cases
# for Histidines, that is, HIA, HIC, HIP, HIQ

protein_letters_3to1 = {'ALA': 'A', 'ARG': 'R', 'ASN': 'N', 'ASP': 'D',
                        'CYS': 'C', 'GLU': 'E', 'GLN': 'Q', 'GLY': 'G',
                        'HIA': 'H', 'HIC': 'H', 'HIP': 'H', 'HIQ': 'H',
                        'HIS': 'H', 'ILE': 'I', 'LEU': 'L', 'LYS': 'K',
                        'MET': 'M', 'PHE': 'F', 'PRO': 'P', 'SER': 'S',
                        'THR': 'T', 'TRP': 'W', 'TYR': 'Y', 'VAL': 'V'}

bwtags = ['1.46', '1.49', '1.50', '1.53', '1.57', '2.42', '2.43', '2.44',
          '2.47', '2.50', '3.34', '3.36', '3.38', '3.40', '3.44', '3.46',
          '3.47', '3.51', '4.50', '4.53', '4.57', '5.54', '5.57', '5.60',
          '6.41', '6.44', '6.47', '6.48', '6.51', '7.38', '7.39', '7.45',
          '7.46', '7.47', '7.50', '7.53']

bwpairs = [('1.46', '7.47'), ('1.49', '7.50'), ('1.50', '2.47'),
           ('1.50', '2.50'), ('1.50', '7.46'), ('1.53', '2.47'),
           ('1.57', '2.44'), ('2.42', '3.46'), ('2.43', '7.53'),
           ('2.50', '7.46'), ('3.34', '4.53'), ('3.34', '4.57'),
           ('3.36', '6.48'), ('3.38', '4.50'), ('3.38', '4.53'),
           ('3.40', '6.44'), ('3.44', '5.54'), ('3.47', '5.57'),
           ('3.51', '5.57'), ('3.51', '5.60'), ('5.54', '6.41'),
           ('6.47', '7.45'), ('6.51', '7.38'), ('6.51', '7.39')]
...
def clustalalign(pdb):
    """
    Align the produced fasta sequence with clustalw to assing
    Ballesteros-Weinstein marks.
    """
    profile1 = s.TEMPLATES_DIR + "/GPCR_inactive_BWtags.aln"
    profile2 = os.path.join(pdb.split(".")[0] + ".fasta")
    bwtagged = os.path.join(pdb.split(".")[0] + "_bw" + ".aln")
...
def getcalphas(pdb):
    ...
    """
    calphaspdb = open(os.path.join(pdb.split(".")[0] + "_CA.pdb"), "w")
    for line in open(pdb, "r"):
        atoms = [a for a in line if line[0:6] == "ATOM  " and line[13:16] == "CA "]
        atomnum = atoms[7:10]
...
def makedisre(pdb):
    ...
    readdisre =  open(s.TEMPLATES_DIR + "/disre.itp", 'r')
    ...
    caid = []
    onlyca = open(os.path.join(pdb.split(".")[0] + "_CA.pdb"), "r")
    ...
    # This part finally writes to the disre.itp file all the information it
    # needs for the atom pairs after they have been mapped from
    # Ballesteros-Weinstein into c-alpha atom numbers in the original pdb
    # given as input.
