...
not only GPCR's.  The package  has been adapted from that described in
...
In  the simplest case, \textbf{py-MEMdyn}
  ...
  assume  that the  file is  called gpcr.pdb.  Thus:
\begin{Verbatim}
run.py -p gpcr.pdb
\end{Verbatim}
  should work!}
...
\item{lig.ff:  we will  refer to  as the  ``force-field file'',  which
  ...
  separate file.}  run.py -p gpcr.pdb -l lig
\end{itemize}
...
complex scenario:
\begin{Verbatim}
run.py -p gpcr.pdb --lig lig --waters hoh --alo alo --cho cho
\end{Verbatim}
...
it. e.g.   the workdir to  run an a2a.pdb simulation  in csb.bmc.uu.se
looks like:
\begin{Verbatim}
\$!/bin/bash   
#!/bin/bash
...
during the equilibration protocol (i.e., detect if atom clashes and so
on exist on your system).

To do this, use the --debug option, like:
\begin{Verbatim}
run.py -p gpcr.pdb --lig lig --waters hoh --debug
\end{Verbatim}
